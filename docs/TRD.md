...
| 버전 | 날짜 | 주요 변경사항 |
|------|------|---------------|
| v0.50.9 | 2026-03-25 | Service CRITICAL `or {}` 수정 17건, 미구현 인터페이스/Dead code 제거, 중복 코드 ~600줄 통합 |
| v0.50.8 | 2026-03-23 | God Class 분해: Handler 7개 서브핸들러, ReplicationManager/BrokerRegistry 구조 개선 |
| v0.50.7 | 2026-03-23 | ISP Consumer Migration (ClusterMetadataPort 분리), rate_limiter 토큰 누수 수정 |
| v0.50.6 | 2026-03-22 | ClusterMetadataPort ISP 5개 서브 인터페이스 분리, 성능 최적화 (zero-alloc, lock 축소) |
| v0.50.5 | 2026-03-22 | Binary protocol DoS 취약점 수정, LoggerPort 인터페이스 추출 (Clean Architecture) |
| v0.50.4 | 2026-03-22 | config.v God File 분리 (5개 파일), i16 producer_epoch overflow 수정 |
| v0.50.3 | 2026-03-22 | manager.v/metrics.v God Class 분리, TOCTOU race 수정, O(1) 버퍼 카운터 |
| v0.50.2 | 2026-03-22 | Binary protocol 음수 길이 검증 (CRITICAL), connection pool race 수정, DRY 추출 |
| v0.50.1 | 2026-03-22 | Stateless 전환: ISR/Rebalance/Leader Election 제거, 바이너리 복제 프로토콜 |
| v0.50.0 | 2026-03-22 | 125 테스트 파일/1,704 함수, `__global` 제거, ISR Manager/Rate Limiting 추가 |
| v0.49.0 | 2026-03-22 | StoragePort ISP 6개 서브 인터페이스 분리, S3 보안 강화, offset race CRITICAL 수정 |
...
pub interface StorageEngine {
    ...
    // Lock interface removed in v0.50.9 -- 파티션 잠금은 각 스토리지 플러그인에서 내부적으로 처리
}
```
...
| **P0** | ~~ISR 관리~~ | v0.50.1에서 제거됨 (stateless 설계) | N/A |
...
│  ├── ISR 관리 -- v0.50.1에서 제거됨 (stateless 설계)                      │
│  └── ElectLeaders API                                                    │
│                                                                          │
│  v0.32.0 - 성능 최적화 I                                                  │
│  ├── io_uring 네트워크 통합                                               │
