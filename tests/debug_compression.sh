...
echo "  Producing message with --compression-type snappy"
...
PRODUCE_OUTPUT=$(echo "$SNAPPY_MESSAGE" | timeout "$TIMEOUT" "$KAFKA_PRODUCER" \
	--bootstrap-server "$BOOTSTRAP_SERVER" \
	--topic "$SNAPPY_TOPIC" \
	--compression-type snappy 2>&1) || true
