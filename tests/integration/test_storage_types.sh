...
start_broker() {
    ...
    echo "Error: Broker failed to start within ${max_attempts}s"
    return 1
}
...
test_memory_large_messages() {
    ...
    local sizes=("100" "1000" "10240" "102400" "1048576")  # 100B, 1KB, 10KB, 100KB, 1MB
    ...
    for size in "${sizes[@]}"; do
        ...
        echo "$message" | timeout "$TIMEOUT" "$KAFKA_PRODUCER" \
            --bootstrap-server "$BOOTSTRAP_SERVER" \
            --topic "$topic" \
            2>/dev/null || { all_passed=false; continue; }
...
test_s3_basic_produce_consume() {
    ...
    sleep 2  # S3 may have higher latency
...
test_s3_large_messages() {
    ...
    # Test larger messages (S3 is better for larger messages)
...
test_s3_data_persistence() {
    ...
    sleep 3  # S3 may have higher latency
...
test_switch_data_integrity() {
    ...
    for msg in "${messages[@]}"; do
        echo "$msg" | timeout "$TIMEOUT" "$KAFKA_PRODUCER" \
            --bootstrap-server "$BOOTSTRAP_SERVER" \
            --topic "$topic1" \
            2>/dev/null || { all_received=false; continue; }
    done
...
run_memory_tests() {
    ...
    run_test "Large Messages (100B-1MB)" test_memory_large_messages "memory"
...
run_s3_tests() {
    ...
    run_test "Large Messages (1KB-1MB)" test_s3_large_messages "s3"
