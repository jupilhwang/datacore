...
// get_numa_capabilities returns NUMA capabilities for current platform
...
// get_numa_topology returns the system's NUMA topology
...
// get_current_node returns the NUMA node for current thread
...
// numa_alloc_fallback provides standard allocation when NUMA not available
...
mut:
	node      int
	buffers   []NumaMemory
	available []int // indices of available buffers
	buf_size  usize
}
...
// get_buffer gets a buffer, preferring local node
...
pub fn (mut p NumaBufferPool) put_buffer(mem NumaMemory) {
	...
	// Check if it's one of our buffers
	mut pool := &p.node_pools[node]
...
// bind_to_node binds current thread to a NUMA node
...
// set_preferred_node sets preferred NUMA node for allocation
...
// set_local_alloc sets thread to allocate from local node
...
// new_numa_array creates a NUMA-aware array on local node
...
// new_numa_array_on_node creates array on specific node
...
// close releases the NUMA array
pub fn (a &NumaArray) close() {
	numa_free(a.memory)
}

// get_ptr returns raw pointer for array operations
pub fn (a &NumaArray) get_ptr() voidptr {
	return a.memory.ptr
}

// get_node returns the NUMA node where array is allocated
