...
pub fn (mut e OTLPExporter) export_metrics_snapshot() {
	reg := get_registry()
	payload := e.build_metrics_payload(reg)
	if payload.len == 0 {
		return
	}
...
fn (e &OTLPExporter) build_metrics_payload(reg &MetricsRegistry) string {
	mut reg_mut := unsafe { reg }
	reg_mut.lock.rlock()
	metrics_copy := reg_mut.metrics.clone()
	reg_mut.lock.runlock()
