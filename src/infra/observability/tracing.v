...
pub fn attr_string(key string, value string) SpanAttribute {
    return SpanAttribute{ key: key, value: SpanValue(value) }
}
...
pub fn (mut s Span) record_error(err IError) {
    ...
    s.add_event('exception',
        attr_string('exception.type', 'error'),
        attr_string('exception.message', err.str())
    )
...
pub fn new_ratio_sampler(ratio f64) RatioSampler {
    return RatioSampler{
        ratio: if ratio < 0 { 0 } else if ratio > 1 { 1 } else { ratio }
    }
}
