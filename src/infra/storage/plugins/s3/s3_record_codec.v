...
/// decode_single_record decodes a single StoredRecord at the given position.
/// Returns the decoded record and the new position, or empty record on boundary error.
fn decode_single_record(data []u8, start_pos int) (StoredRecord, int) {
	mut pos := start_pos
	if pos + 20 > data.len {
		return StoredRecord{}, -1
	}
	...
	headers, new_pos := decode_record_headers(data, pos)
	pos = new_pos
...
/// decode_record_headers decodes the headers map from binary data at the given position.
fn decode_record_headers(data []u8, start_pos int) (map[string][]u8, int) {
	mut pos := start_pos
...
fn decode_stored_records(data []u8) []StoredRecord {
	...
	for pos + 4 <= data.len {
		...
		for _ in 0 .. record_count {
			rec, new_pos := decode_single_record(data, pos)
			if new_pos < 0 {
				return records
			}
			records << rec
