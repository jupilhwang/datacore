...
pub fn (r &ZeroCopyReader) remaining() int { return r.reader.remaining() }
pub fn (r &ZeroCopyReader) position() int { return r.reader.position }
pub fn (r &ZeroCopyReader) has_remaining() bool { return r.reader.has_remaining() }
...
pub fn (r &SimpleReader) remaining() int { return r.data.len - r.pos }
pub fn (r &SimpleReader) position() int { return r.pos }
pub fn (r &SimpleReader) has_remaining() bool { return r.pos < r.data.len }

pub fn (mut r SimpleReader) read_i8() !i8 {
    if r.pos >= r.data.len { return error('eof') }
    val := r.data[r.pos]
    r.pos++
    return i8(val)
}
...
pub fn (mut r SimpleReader) read_uvarint() !u64 {
    ...
    for {
        ...
        if b & 0x80 == 0 { break }
        shift += 7
        if shift >= 64 { return error('uvarint overflow') }
    }
    return result
}
...
pub fn (mut r SimpleReader) read_bytes(len int) ![]u8 {
    if r.pos+len > r.data.len { return error('eof') }
...
pub fn (mut r SimpleReader) skip(n int) ! {
    if r.pos+n > r.data.len { return error('eof') }
    r.pos += n
}
