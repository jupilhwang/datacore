...
pub fn (f Frame) response_to_bytes(api_key ApiKey, version i16) []u8 {
    ...
    if is_flexible {
        return build_flexible_response(f.header.correlation_id(), body_bytes)
    }
...
pub:
    ...
    min_version         i16
    max_version         i16
    flexible_version    i16  // Version where flexible encoding starts (-1 if never)
}

// Get API version info for all supported APIs
pub fn get_api_version_info() []ApiVersionInfo {
    return [
        ApiVersionInfo{.produce, 0, 9, 9},
        ApiVersionInfo{.fetch, 0, 13, 12},
        ApiVersionInfo{.list_offsets, 0, 7, 6},
        ApiVersionInfo{.metadata, 0, 12, 9},
        ApiVersionInfo{.offset_commit, 0, 8, 8},
        ApiVersionInfo{.offset_fetch, 0, 8, 6},
        ApiVersionInfo{.find_coordinator, 0, 4, 3},
        ApiVersionInfo{.join_group, 0, 9, 6},
        ApiVersionInfo{.heartbeat, 0, 4, 4},
        ApiVersionInfo{.leave_group, 0, 5, 4},
        ApiVersionInfo{.sync_group, 0, 5, 4},
        ApiVersionInfo{.describe_groups, 0, 5, 5},
        ApiVersionInfo{.list_groups, 0, 4, 3},
        ApiVersionInfo{.api_versions, 0, 4, 3},
        ApiVersionInfo{.create_topics, 0, 7, 5},
        ApiVersionInfo{.delete_topics, 0, 6, 4},
        ApiVersionInfo{.delete_records, 0, 2, 2},
        ApiVersionInfo{.init_producer_id, 0, 4, 2},
        ApiVersionInfo{.describe_configs, 0, 4, 4},
        ApiVersionInfo{.alter_configs, 0, 2, 2},
        ApiVersionInfo{.create_partitions, 0, 3, 2},
        ApiVersionInfo{.delete_groups, 0, 2, 2},
        ApiVersionInfo{.describe_cluster, 0, 1, 0},
        ApiVersionInfo{.consumer_group_heartbeat, 0, 0, 0},
        ApiVersionInfo{.consumer_group_describe, 0, 0, 0},
        ApiVersionInfo{.share_group_heartbeat, 0, 0, 0},
        ApiVersionInfo{.share_group_describe, 0, 0, 0},
        ApiVersionInfo{.share_fetch, 0, 0, 0},
        ApiVersionInfo{.share_acknowledge, 0, 0, 0},
    ]
}

// Check if version is flexible for a given API
pub fn is_version_flexible(api_key ApiKey, version i16) bool {
    for info in get_api_version_info() {
        if info.api_key == api_key {
            return info.flexible_version >= 0 && version >= info.flexible_version
        }
    }
    return false
}
