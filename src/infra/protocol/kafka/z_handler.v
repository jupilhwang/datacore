	...
	for t in req.topic_data {
		...
		if version >= 13 && t.topic_id.len == 16 {
			if topic_meta := h.storage.get_topic_by_id(t.topic_id) {
				...
				// Topic ID not found - return error for all partitions
...
fn (mut h Handler) handle_describe_configs(body []u8, version i16) ![]u8 {
	mut reader := new_reader(body)
	req := parse_describe_configs_request(mut reader, version, is_flexible_version(.describe_configs, version))!
