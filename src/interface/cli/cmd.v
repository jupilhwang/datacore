...
//
// CLI application for managing and operating the DataCore broker.
// Provides commands for broker start/stop, topic management,
// message production/consumption, etc.
//
// Key features:
// - Broker start, stop, status check
...
/// App is a struct representing the CLI application.
pub struct App {
pub:
...
pub fn (app &App) print_help() {
	...
	println('  \x1b[32mgroup delete\x1b[0m          Delete a consumer group')
	println('  \x1b[32mgroup reset-offset\x1b[0m    Reset consumer group offset')
	println('  \x1b[32mshare-group list\x1b[0m      List share groups')
	println('  \x1b[32mshare-group describe\x1b[0m  Describe a share group')
	println('  \x1b[32mshare-group delete\x1b[0m    Delete a share group')
	println('  \x1b[32macl create\x1b[0m            Create an ACL binding')
	println('  \x1b[32macl list\x1b[0m              List ACL bindings')
	println('  \x1b[32macl delete\x1b[0m            Delete an ACL binding')
	println('  \x1b[32mcluster describe\x1b[0m      Show cluster information')
	println('  \x1b[32mcluster brokers\x1b[0m       List all brokers')
	println('  \x1b[32mcluster config\x1b[0m        Show cluster configuration')
	println('  \x1b[32moffset get\x1b[0m            Get committed offset')
	println('  \x1b[32moffset set\x1b[0m            Set committed offset')
	println('  \x1b[32mhealth\x1b[0m                Full health check')
	println('  \x1b[32mreadiness\x1b[0m             Readiness probe')
	println('  \x1b[32mliveness\x1b[0m              Liveness probe')
	...
	println('  ${app.name} topic alter my-topic --new-partitions 6')
	println('  ${app.name} group reset-offset my-group --topic my-topic --to-earliest')
	println('  ${app.name} acl create --principal User:alice --resource my-topic --operation Read')
	println('  ${app.name} cluster describe')
	println('  ${app.name} health')
...
/// CliOptions is a struct holding CLI options.
...
/// get_config_path retrieves the configuration file path from arguments.
...
/// write_pid writes the current process PID to a file.
pub fn write_pid(path string) ! {
	pid := os.getpid()
	os.write_file(path, '${pid}')!
}

/// read_pid reads a PID from a file.
...
/// check_pid_running checks whether the process with the given PID is running.
...
/// print_shutdown prints the shutdown message.
pub fn print_shutdown() {
	println('')
	println('\x1b[33m⚡\x1b[0m Shutting down broker...')
}

/// print_shutdown_complete prints the shutdown completion message.
pub fn print_shutdown_complete() {
	println('\x1b[32m✓\x1b[0m Broker stopped.')
}

/// BrokerStatus is a struct holding broker status information.
...
// format_duration converts a Duration to a human-readable format.
...
/// print_done prints a completion message.
pub fn print_done() {
	println(' \x1b[32m✓\x1b[0m')
}

/// print_failed prints a failure message.
