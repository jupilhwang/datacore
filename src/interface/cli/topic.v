...
pub fn parse_topic_options(args []string) TopicOptions {
    ...
    for i < args.len {
        match args[i] {
            '--bootstrap-server', '-b' {
                if i + 1 < args.len {
                    opts = TopicOptions{ ...opts, bootstrap_server: args[i + 1] }
                    i += 1
                }
            }
            '--topic', '-t' {
                if i + 1 < args.len {
                    opts = TopicOptions{ ...opts, topic: args[i + 1] }
                    i += 1
                }
            }
            '--partitions', '-p' {
                if i + 1 < args.len {
                    opts = TopicOptions{ ...opts, partitions: args[i + 1].int() }
                    i += 1
                }
            }
            '--replication-factor', '-r' {
                if i + 1 < args.len {
                    opts = TopicOptions{ ...opts, replication: args[i + 1].int() }
                    i += 1
                }
            }
            else {
                ...
                if !args[i].starts_with('-') && opts.topic.len == 0 {
                    opts = TopicOptions{ ...opts, topic: args[i] }
                }
            }
        }
        i += 1
...
pub fn run_topic_create(opts TopicOptions) ! {
    ...
    request := build_create_topic_request(opts.topic, opts.partitions, opts.replication, opts.timeout_ms)
...
fn parse_metadata_response_topics(response []u8) []TopicInfo {
    ...
    if pos >= response.len { return topics }
    ...
    for _ in 0 .. topic_count {
        ...
        pos += 1
        if pos + name_len > response.len { break }
