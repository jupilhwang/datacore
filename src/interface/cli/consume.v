...
pub fn parse_consume_options(args []string) ConsumeOptions {
    ...
    for i < args.len {
        match args[i] {
            '--bootstrap-server', '-b' {
                if i + 1 < args.len {
                    opts = ConsumeOptions{ ...opts, bootstrap_server: args[i + 1] }
                    i += 1
                }
            }
            '--topic', '-t' {
                if i + 1 < args.len {
                    opts = ConsumeOptions{ ...opts, topic: args[i + 1] }
                    i += 1
                }
            }
            '--partition', '-p' {
                if i + 1 < args.len {
                    opts = ConsumeOptions{ ...opts, partition: args[i + 1].int() }
                    i += 1
                }
            }
            '--group', '-g' {
                if i + 1 < args.len {
                    opts = ConsumeOptions{ ...opts, group: args[i + 1] }
                    i += 1
                }
            }
            '--offset', '-o' {
                if i + 1 < args.len {
                    opts = ConsumeOptions{ ...opts, offset: args[i + 1] }
                    i += 1
                }
            }
            '--max-messages', '-n' {
                if i + 1 < args.len {
                    opts = ConsumeOptions{ ...opts, max_messages: args[i + 1].int() }
                    i += 1
                }
            }
            '--from-beginning' {
                opts = ConsumeOptions{ ...opts, from_beginning: true, offset: 'earliest' }
            }
            else {
                // First positional arg might be topic
                if !args[i].starts_with('-') && opts.topic.len == 0 {
                    opts = ConsumeOptions{ ...opts, topic: args[i] }
                }
            }
        }
        i += 1
...
pub fn run_consume(opts ConsumeOptions) ! {
    ...
    for {
        ...
        // Build Fetch request
        request := build_fetch_request(opts.topic, opts.partition, fetch_offset, 1048576, opts.timeout_ms)
