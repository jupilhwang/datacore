...
fn make_metrics() d.ReplicationMetrics {
	return d.new_replication_metrics()
}
...
fn test_thread_safe_concurrent_updates() {
	// Allocate on heap so spawn closures share the same instance
	mut m := &d.ReplicationMetrics{
		window_start_time: time.now().unix_milli()
		lag_samples:       []f64{len: 1024, init: 0.0}
	}
	goroutines := 20
	...
	for _ in 0 .. goroutines {
		threads << spawn fn [mut m, ops_per_goroutine] () {
...
fn test_partition_metrics_initialization() {
	pm := d.new_partition_metrics('test-topic', 0)
...
fn test_partition_metrics_sent_and_acked() {
	mut pm := d.new_partition_metrics('orders', 3)
	pm.record_sent(256)
	pm.record_sent(512)
	pm.record_acked()
...
fn test_partition_metrics_avg_lag() {
	mut pm := d.new_partition_metrics('events', 1)
	pm.record_lag(10.0)
	pm.record_lag(20.0)
	pm.record_lag(30.0)
	avg := pm.avg_lag_ms()
...
fn test_partition_metrics_avg_lag_no_samples() {
	mut pm := d.new_partition_metrics('empty-topic', 0)
	avg := pm.avg_lag_ms()
