...
pub fn (e &ProtobufEncoder) decode(data []u8, schema domain.Schema) ![]u8 {
    ...
    mut reader := ProtoReader{ data: data, pos: 0 }
...
fn proto_decode_fixed32(mut reader ProtoReader) !u32 {
    ...
    result := u32(reader.data[reader.pos]) |
...
fn proto_decode_fixed64(mut reader ProtoReader) !u64 {
    ...
    result := u64(reader.data[reader.pos]) |
...
fn parse_protobuf_schema(schema_str string) !ProtoMessage {
    ...
    if msg_idx := schema_str.index('message') {
        ...
        mut end := pos
        for end < schema_str.len && schema_str[end] != ` ` && schema_str[end] != `{` && schema_str[end] != `\n` {
            end += 1
        }
    ...
    for line in lines {
        ...
        if trimmed.len == 0 || trimmed.starts_with('//') || trimmed.starts_with('message') || 
           ...
           trimmed.starts_with('syntax') || trimmed.starts_with('package') || trimmed.starts_with('option') {
            continue
        }
...
fn parse_proto_field(line string) ?ProtoField {
    ...
    tokens := type_name.split(' ').filter(fn (s string) bool { return s.len > 0 })
