...
pub fn (e &AvroEncoder) decode(data []u8, schema domain.Schema) ![]u8 {
    ...
    mut reader := AvroReader{ data: data, pos: 0 }
...
fn decode_float(mut reader AvroReader) !f32 {
    ...
    bits := u32(reader.data[reader.pos]) |
            (u32(reader.data[reader.pos + 1]) << 8) |
            (u32(reader.data[reader.pos + 2]) << 16) |
            (u32(reader.data[reader.pos + 3]) << 24)
    reader.pos += 4
...
fn decode_double(mut reader AvroReader) !f64 {
    ...
    bits := u64(reader.data[reader.pos]) |
            (u64(reader.data[reader.pos + 1]) << 8) |
            (u64(reader.data[reader.pos + 2]) << 16) |
            (u64(reader.data[reader.pos + 3]) << 24) |
            (u64(reader.data[reader.pos + 4]) << 32) |
            (u64(reader.data[reader.pos + 5]) << 40) |
            (u64(reader.data[reader.pos + 6]) << 48) |
            (u64(reader.data[reader.pos + 7]) << 56)
    reader.pos += 8
    ...
    for field in schema.fields {
        ...
        if field.is_union {
            if is_json_null(field_value) {
                ...
                field_schema := AvroSchema{ schema_type: actual_type }
                encoded := e.encode_value(field_value, field_schema)!
                result << encoded
            }
        } else {
            // Simple field encoding
            field_schema := AvroSchema{ schema_type: field.field_type }
            encoded := e.encode_value(field_value, field_schema)!
            result << encoded
        }
    }
...
fn (e &AvroEncoder) encode_array(json_str string, schema AvroSchema) ![]u8 {
    ...
    // Encode each item
    item_schema := AvroSchema{ schema_type: schema.items_type }
    for item in items {
...
fn (e &AvroEncoder) encode_map(json_str string, schema AvroSchema) ![]u8 {
    ...
    // Encode each entry
    value_schema := AvroSchema{ schema_type: schema.values_type }
...
fn (e &AvroEncoder) encode_union(json_str string, schema AvroSchema) ![]u8 {
    ...
    for i, t in schema.union_types {
        if t != 'null' {
            type_schema := AvroSchema{ schema_type: t }
...
fn (e &AvroEncoder) decode_record(mut reader AvroReader, schema AvroSchema) !string {
    ...
    for field in schema.fields {
        ...
        if field.is_union {
            ...
            type_schema := AvroSchema{ schema_type: union_type }
...
fn extract_json_field(json_str string, field_name string) ?string {
    ...
    for pos < json_str.len && (json_str[pos] == `:` || json_str[pos] == ` ` || json_str[pos] == `\t` || json_str[pos] == `\n`) {
        pos += 1
    }
...
fn read_json_value(s string, start int) ?string {
    ...
    if c == `"` {
        ...
        // Number
        mut end := start
        for end < s.len && (s[end] == `-` || s[end] == `+` || s[end] == `.` || s[end] == `e` || s[end] == `E` || (s[end] >= `0` && s[end] <= `9`)) {
            end += 1
        }
        return s[start..end]
    }
...
fn parse_json_array(s string) ?[]string {
    ...
    for pos < trimmed.len - 1 {
        // Skip whitespace
        for pos < trimmed.len && (trimmed[pos] == ` ` || trimmed[pos] == `\t` || trimmed[pos] == `\n` || trimmed[pos] == `,`) {
            pos += 1
        }
...
fn parse_json_map(s string) ?map[string]string {
    ...
    for pos < trimmed.len - 1 {
        ...
        for pos < trimmed.len && (trimmed[pos] == `:` || trimmed[pos] == ` ` || trimmed[pos] == `\t` || trimmed[pos] == `\n`) {
            pos += 1
        }
