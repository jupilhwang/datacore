...
// register registers a new schema for a subject
// Returns the schema ID (existing if duplicate, new if unique)
...
// normalize_schema removes whitespace for consistent fingerprinting
fn normalize_schema(schema_str string) string {
	return schema_str.replace(' ', '').replace('\n', '').replace('\t', '').replace('\r',
		'')
}

// compute_fingerprint computes MD5 hash of schema string
